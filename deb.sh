...
clear
echo "#############################################################"
echo "# Linux + Apache + Nginx + MySQL/MariaDB + MOODLE + PHP Auto Install Script"
...
echo "Please choose database software! (1:mariadb,2:mysql)"
read -p "(Default: 1):" DBSOFTWARE
if [ -z $DBSOFTWARE ]; then
DBSOFTWARE="3"
fi
echo "---------------------------"
echo "You choose = $DBSOFTWARE"
echo "---------------------------"
echo ""
...
apt-get -y --force-yes install libxmlrpc-core-c3 libxmlrpc-c3-dev
...
apt-get -y --force-yes install libc-client2007e libc-client2007e-dev libicu-dev
apt-get -y autoremove
apt-get -fy install
...
echo "---------- Apache config ----------"
	...
	if [ "$SOFTWARE" = "3" ]; then
		...
		if [ ! -s mod_rpaf-*.tar.gz ]; then
			...
			BACKUP_RPAF_LINK="hhttp://www.hnsssy.com/download/software/mod_rpaf-latest.tar.gz"
...
if [ "$M_VERSION" = "1" ]; then
 ...
 if [ ! -s moolde-*-26.tgz ]; then
   LATEST_moodle_LINK="http://www.hnsssy.com/download/moodle/moodle-latest-26.tgz"
   BACKUP_moodle_LINK="http://download.moodle.org/download.php/stable26/moodle-latest-26.tgz"
...
if [ "$M_VERSION" = "2" ]; then
    ...
    if [ ! -s moolde-*-27.tgz ]; then
       LATEST_moodle_LINK="http://www.hnsssy.com/download/moodle/moodle-latest-27.tgz"
       BACKUP_moodle_LINK="http://download.moodle.org/download.php/stable27/moodle-latest-27.tgz"
       Extract ${LATEST_moodle_LINK} ${BACKUP_moodle_LINK}
       mkdir -p $WEBROOT/moodle
       mv * $WEBROOT/moodle
    else
